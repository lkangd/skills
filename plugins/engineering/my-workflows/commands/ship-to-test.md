...
- **`STATUS: OK`**: Read `SUMMARY` and the full trigger output for each environment, then report. Confirm that every non-skipped environment has `status: verified`; distinguish `trigger_status` (accepted/not needed) from `verification_status: verified`, and distinguish `verification_mode: observed_after_trigger` from `already_present`. Any `NOTE:` lines (skipped environments, reused merged MR, invalidated stale verification) must be reflected in the report. If the output contains an `ASK_REGISTER_GLOBAL` line (a reviewer was added via argument this run), ask the user during the report whether to register that reviewer as `*` (all projects); if they agree, run the `register-global` command given on that line.
